...
public class ImageLayoutAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
		...
		@Override public void onScrolled(RecyclerView recycler, int dx, int dy) {
            ...
            recycler.postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    if (!loading && totalItemCount >= currentPos * visibleThreshold && totalItemCount <= (lastVisibleItem + visibleThreshold))
                        requestRefresh(true, currentPos + 1, 1);
                }
            }, 2000);
        }
	};
    ...
    public boolean requestRefresh(boolean showProgressBar) {
        return requestRefresh(showProgressBar, 1, 1);
    }

    public boolean requestRefresh(boolean showProgressBar, int start, int count) {
        ...
        new DownloadAsyncTask().execute(start, count);
        return true;
    }
    ...
    public class DownloadAsyncTask extends AsyncTask<Integer, Void, ArrayList<Image>> {
        ...
        protected ArrayList<Image> doInBackground(Integer... params) {
            ...
            try { //Parse thumbnails.
                int index;
                for (index = params[0]; images.size() < params[1] * visibleThreshold; index++) {
